// Copyright 2013 Alex Reece.  Modified by a Paper #67 Author (2016-2018).
//
...
// Paper #67 Author - Was previously GiB/s, can switch back if needed.
...
void timeitp(void (*function)(void*, size_t), char* name) {
  double min = INFINITY;
  // Paper #67 Author - add max, mean, delta, delta2, and M2 to calculate more complete statistics
  double max = 0;
  double mean = 0;
  double delta = 0;
  double delta2 = 0;
...
#pragma omp master
    ...
    // Paper #67 Author - Convert to bw before collecting statistics
...
int main() {
  ...
  // Paper #67 Author - Print extension information
  printf("AVX:\t\t\t\t");
#ifdef __AVX__
  printf("enabled\n");
#else
